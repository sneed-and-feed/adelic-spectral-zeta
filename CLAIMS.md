...
* **Adèlic Topological Quantum Error Correction** ([AdelicTopologicalQEC.lean](formalization/Formalization/AdelicTopologicalQEC.lean))
  * *Claim:* The adèlic parity-check matrix $H_{p,k} = 1 \iff p \mid k$ defines a valid syndrome map over the prime-indexed stabilizer lattice. A thermal bit-flip error threshold $p_c$ exists such that for $p < p_c$, any efficient decoder drives the logical failure probability to zero as $N \to \infty$.
