...
3. **Sub-Leading Circle Bound on Survival Probabilities:** For any non-empty stopping set $A \subset \mathbb{Z}/2^n\mathbb{Z}$ (such as the Terras descent set $A = \{y : y \lt x_0\}$ or dyadic half-space $[0, 2^{n-1}-1]$), the first stopping time $T = \inf \{ t \ge 1 : X_t \in A \}$ has survival probability bounded by the sub-leading circle radius:

$$P(T > t) \le C \cdot \left(\frac{\sqrt{2}}{2}\right)^t = C \cdot 2^{-t/2}$$

4. **Riho Terras Stopping Time Moments:** The moment generating function $M_T(s) = \mathbb{E}[e^{s T}]$ is analytic in the disk $\text{Re}(s) \lt \frac{1}{2} \ln 2$, directly yielding explicit finiteness and closed-form bounds for all polynomial moments $\mathbb{E}[T^k] \lt \infty$ ($k \in \mathbb{N}$) directly from the spectral gap $\Delta = 2 - \sqrt{2}$.
...
All theoretical theorems are validated numerically to machine precision ($\lt 10^{-15}$) and against $N = 250,000$ Monte-Carlo random walk trajectories in `experiments/collatz_markov_stopping_times.py`.
...
For $m \ge 3$, $|2^{-t} W_t(k)| \le C_m 2^{-t(1 - 2^{-(m-1)})} \le C_m 2^{-3t/4} \lt 2^{-t/2}$.  
...
Since $\delta_{x_0} - \pi \in L^2_0$ and $\|\delta_{x_0} - \pi\|_2 = \sqrt{1 - 2^{-n}} \lt 1$, Theorem 4.2 implies:
...
Let $A \subset \mathbb{Z}/2^n\mathbb{Z}$ be a non-empty stopping target set (e.g. the Terras descent set $A = \{ y \mid y \lt x_0 \}$ or the dyadic lower half $A = [0, 2^{n-1}-1]$).  
...
*Let $A \subset \mathbb{Z}/2^n\mathbb{Z}$ be any non-empty set with non-trivial measure $\pi(A) \gt 0$. The spectral radius of $Q = P_{A^c, A^c}$ is strictly bounded by the sub-leading circle radius:*
...
In his seminal 1976 work (*Acta Arithmetica* 30), Riho Terras defined the stopping time $\sigma_\infty(x) = \inf \{ k \ge 1 \mid T^{(k)}(x) \lt x \}$ and proved that almost all integers have a finite stopping time.
...
*Proof.*  
Using the identity $\mathbb{E}[T^k] = \sum_{t=0}^\infty ((t+1)^k - t^k) P(T \gt t)$:

$$M_T(s) = \mathbb{E}[e^{s T}] = \sum_{t=0}^\infty e^{s(t+1)} P(T = t+1) = 1 + (e^s - 1) \sum_{t=0}^\infty e^{s t} P(T > t)$$

Since $P(T \gt t) \le C \cdot 2^{-t/2} = C e^{-t (\ln 2)/2}$, the series converges absolutely for $e^{\text{Re}(s)} 2^{-1/2} \lt 1 \iff \text{Re}(s) \lt \frac{1}{2} \ln 2$.  
...
*Moreover, due to the uniform spectral gap $\gamma = \frac{2 - \sqrt{2}}{2} \gt 0$, the stopping time $T_n$ satisfies the sub-Gaussian concentration inequality:*

$$P\left( \left| T_n - \mathbb{E}[T_n] \right| \ge \lambda \sqrt{n} \right) \le 2 \exp\left( - \frac{c \lambda^2 \gamma}{1 + \gamma} \right)$$

*for an absolute constant $c \gt 0$.*
...
| **Max Absolute Survival Error** | $\max_t |P_{\text{MC}}(T \gt t) - P_{\text{Exact}}(T \gt t)|$ | — | $\mathbf{9.26 \times 10^{-4}}$ |
