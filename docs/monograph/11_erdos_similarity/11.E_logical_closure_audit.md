...
- **Topologies and Convergence:** Strong resolvent convergence in the operator topology on $`L^2(X_\infty)`$ for $`\mathrm{Re}(z) \le -(\lambda + 1)`$. Pointwise monotone convergence of cylinder sets $`\mathcal{E}_d \downarrow \mathcal{E}`$.
