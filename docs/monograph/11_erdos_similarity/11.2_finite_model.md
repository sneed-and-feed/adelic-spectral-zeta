...
2. **Multiplicative Cycle**: Because $`v_p(q) = 0`$ and $`q \not\equiv 1 \pmod p`$, the sequence elements $`q^{-n} \bmod p`$ cycle within the multiplicative group $`(\mathbb{Z}/p\mathbb{Z})^\times`$. The period of this sequence is the order $`d = \mathrm{ord}_p(q)`$ of $q$ modulo $p$. Since $`q \not\equiv 1 \pmod p`$, we have $`d \ge 2`$. The residues form a multiplicative subgroup:
