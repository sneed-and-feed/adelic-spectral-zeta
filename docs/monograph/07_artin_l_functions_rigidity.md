...
## 5.7 Numerical Verification of Off-Critical Line Rigidity

To numerically explore the spectral rigidity of the Artin $L$-function away from the critical line $\sigma = 1/2$, we evaluate the minimal physical eigenvalue $|\lambda_{\text{min}}|$ of the compressed boundary operator across a 2D parameter sweep: $\sigma \in [0.1, 0.9]$ and $t \in [5, 25]$. 

Once the trivial zero eigenvalue of the projection kernel is removed, the scan reveals that the smallest physical eigenvalue magnitude $|\lambda_{\text{min}}|$ is strictly positive off the critical line. Within the scanned domain, the global minimum eigenvalue magnitude off the critical line is found to be strictly bounded away from zero ($\min_{\sigma \neq 1/2} |\lambda_{\text{min}}| \gt 0.05$), demonstrating the numerical stability and positivity of the secular gap for this model representation.
