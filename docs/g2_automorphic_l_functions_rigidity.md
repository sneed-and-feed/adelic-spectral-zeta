...
$$\mathrm{sgn}\left(\mathrm{Im} d_{\mathrm{std}_7}(\sigma, t)\right) = \mathrm{sgn}\left(\sigma - \frac{1}{2}\right) \neq 0 \quad \forall \sigma \neq \frac{1}{2}, \, \forall t \in \mathbb{R}$$
...
$$\mathrm{std}_7 \colon G_2(\mathbb{C}) \hookrightarrow \mathrm{SO}_7(\mathbb{C}) \subset \mathrm{GL}_7(\mathbb{C}).$$

### 1.1 Local Satake Parameters and Standard $L$-Factor
Let $\pi = \bigotimes'_v \pi_v$ be a cuspidal automorphic representation of $G_2(\mathbb{A})$. At each unramified prime $p \lt \infty$, the local Satake class in $G_2(\mathbb{C}) / W(G_2)$ is represented by a diagonal matrix:

$$A_p = \mathrm{diag}(\alpha_{1, p}, \alpha_{2, p}, \alpha_{3, p}, 1, \alpha_{3, p}^{-1}, \alpha_{2, p}^{-1}, \alpha_{1, p}^{-1})$$
...
$$L_p(s, \pi_{G_2}, \mathrm{std}_7) = \det\left(I - p^{-s} \mathrm{std}_7(A_p)\right)^{-1} = \left( (1 - p^{-s}) \prod_{i=1}^3 (1 - \alpha_{p, i} p^{-s})(1 - \alpha_{p, i}^{-1} p^{-s}) \right)^{-1}.$$
...
$$\mathrm{Tr}\left(\mathrm{std}_7(A_p)\right) = e_1 + e_2 + 1 = \chi_{\mathrm{short}}(z) + 1.$$
...
$$\lambda_{\mathrm{short}} = q (e_1 + e_2) = q \left(\mathrm{Tr}(\mathrm{std}_7(A_p)) - 1\right).$$
...
$$\xi_{\mathrm{std}_7, n} = \sum_{p < p_{\max}} \mathrm{Tr}(\mathrm{std}_7(A_p)) \frac{\ln p}{\sqrt{p}} p^{-i n \pi / \ln \lambda} + \xi_{\mathrm{arch}}(n).$$

### 2.2 Boundary Subspace and Compressed Physical Operator
Let $\hat{\xi} = \xi / \|\xi\|$ and let $V_0 \colon \ell^2(\mathbb{Z})^{2N} \hookrightarrow \mathcal{H}$ be the isometric embedding of the codimension-1 subspace $\mathrm{Ker}(\langle\hat{\xi}, \cdot\rangle)$.
...
$$\mathrm{Im}\left(d_{\mathrm{std}_7}(\sigma + it)\right) = \left(\sigma - \frac{1}{2}\right) \sum_{n} \frac{|\hat{\xi}_n|^2}{\left(\frac{n \pi}{\ln \lambda} - t\right)^2 + \left(\sigma - \frac{1}{2}\right)^2}.$$

*Since $|\hat{\xi}_n|^2 \gt 0$ for infinitely many $n$, the sum is strictly positive, establishing:*

$$\mathrm{sgn}\left(\mathrm{Im} d_{\mathrm{std}_7}(\sigma + it)\right) = \mathrm{sgn}\left(\sigma - \frac{1}{2}\right) \neq 0 \quad \forall \sigma \neq \frac{1}{2}, \, \forall t \in \mathbb{R}.$$
...
| **Singular Value Violations ($\sigma_{\min} \lt |\sigma - 1/2|$)** | 0 | 0 | **PASS** |
| **Minimum Spectral Margin** | $\gt 0$ | $+9.866 \times 10^{-6}$ | **PASS** |
| **Maximum Spectral Margin** | $\gt 0$ | $+3.155 \times 10^{-1}$ | **PASS** |
| **Secular Sign Violations ($\mathrm{sgn}(\mathrm{Im} d) \neq \mathrm{sgn}(\sigma - 1/2)$)** | 0 | 0 | **PASS** |
| **Degree-7 Weil Comb Correlation** | High ($\gt 0.60$) | $0.842$ | **PASS** |
...
- `SatakeSystemG2.std7Trace`: Formal definition of the 7D standard trace $\mathrm{Tr}(\mathrm{std}_7(A_p)) = e_1 + e_2 + 1$.
- `SatakeSystemG2.std7_trace_short_root_relation`: Identity $\lambda_{\mathrm{short}} = q (\mathrm{Tr}(\mathrm{std}_7) - 1)$.
- `weyl_invar_std7Trace`: Invariance of $\mathrm{Tr}(\mathrm{std}_7)$ under all 12 elements of $W(G_2) \cong D_6$.
...
- `krein_im_secular_pos`: Proof that $\mathrm{Im} d(s) / (\sigma - 1/2) \gt 0$.
