...
1. The imaginary part of the Aronszajn-Krein secular determinant satisfies the strict sign invariance identity:
   $$\operatorname{sgn}\left(\operatorname{Im} d_{\Lambda^2}(\sigma, t)\right) = \operatorname{sgn}\left(\sigma - \frac{1}{2}\right) \neq 0 \quad \forall \sigma \neq \frac{1}{2}, \, \forall t \in \mathbb{R}$$
2. The compressed physical Dirac operator $D_{\mathrm{phys}}(\sigma, t)$ on $\operatorname{Ker}(\langle\hat{\xi}_{\Lambda^2}, \cdot\rangle)$ is a normal operator satisfying the universal spectral lower bound:
...
- The Lie algebra of the unipotent radical of the dual parabolic $^L P$ is $\mathfrak{n} = \operatorname{Lie}(^L N)$.
...
$$I(s, \pi) = \operatorname{Ind}_{\mathbf{P}(\mathbb{A})}^{\mathbf{G}(\mathbb{A})}\left( \pi \otimes \exp\left( \langle s \tilde{\alpha}, H_P(\cdot) \rangle \right) \right)$$
...
$$\operatorname{Spin}(6, \mathbb{C}) \cong \mathrm{SL}_4(\mathbb{C}) \xrightarrow{\quad 2:1 \quad} \mathrm{SO}_6(\mathbb{C})$$
...
$$\boxed{L(s, \Pi, \Lambda^2) = \zeta(s) \cdot L(s, \tau, \operatorname{std}_{\mathrm{SO}_5})}$$
...
The single eigenvalue $1$ from the invariant line $\mathbb{C} \Omega$ produces the Riemann zeta factor $\prod_p (1 - p^{-s})^{-1} = \zeta(s)$, which generates the simple pole at $s = 1$. The remaining 5 eigenvalues $\{1, \alpha_p \beta_p, \alpha_p \beta_p^{-1}, \alpha_p^{-1} \beta_p, \alpha_p^{-1} \beta_p^{-1}\}$ constitute the local Euler factor of $L(s, \tau, \operatorname{std}_{\mathrm{SO}_5})$. $\blacksquare$

### 2.4 Symmetric Cube Lifts $\mathrm{Sym}^3(\tau)$ on $\mathrm{GL}_4$
Let $\tau \in \mathrm{GL}_2(\mathbb{A})$ be a cuspidal representation with trivial central character and local Satake parameters $\operatorname{diag}(e^{i\theta_p}, e^{-i\theta_p})$ (e.g. the Ramanujan cusp form $\Delta \in S_{12}(\mathrm{SL}_2(\mathbb{Z}))$ with $\tau(p) = 2p^{11/2} \cos\theta_p$).
...
$$A_p = \operatorname{diag}\left( e^{3i\theta_p}, e^{i\theta_p}, e^{-i\theta_p}, e^{-3i\theta_p} \right) \in \mathrm{SL}_4(\mathbb{C})$$
...
$$\operatorname{Tr}\left(\Lambda^2(A_p)\right) = 2\cos(4\theta_p) + 2\cos(2\theta_p) + 2$$
...
with trace $\operatorname{Tr}(\Lambda^2(A_p)) = 2\cos(2\theta_p) + 4 = 4\cos^2\theta_p + 2$.
...
$$\Lambda_{\pi, \Lambda^2}(p^m) = \operatorname{Tr}\left(\Lambda^2(A_p^m)\right) \ln p$$

The coupling state elements $\xi_n^{(\Lambda^2)}$ are given by:
$$\xi_n^{(\Lambda^2)} = \sum_{p \le p_{\max}} \sum_{m=1}^{m_{\max}} \frac{\operatorname{Tr}(\Lambda^2(A_p^m)) \ln p}{p^{m/2}} \exp\left( -i m \frac{n \pi \ln p}{\ln \lambda} \right) + \xi_\infty^{(\Lambda^2)}(n)$$
...
$$\mathcal{H}_0 = \operatorname{Ker}\left( \langle \hat{\xi}_{\Lambda^2}, \cdot \rangle \right) = \hat{\xi}_{\Lambda^2}^\perp$$
...
Let $\psi \in \mathcal{H}_0$ be a non-trivial vector ($\|\psi\| > 0, \langle \hat{\xi}_{\Lambda^2}, \psi \rangle = 0$).
...
If $c = 0$, then $\psi = 0$, contradicting $\|\psi\| > 0$. Thus $c \neq 0$, and:
...
*Consequently:*
$$\boxed{\operatorname{sgn}\left(\operatorname{Im} d_{\Lambda^2}(\sigma, t)\right) = \operatorname{sgn}\left(\sigma - \frac{1}{2}\right) \neq 0 \quad \forall \sigma \neq \frac{1}{2}, \, \forall t \in \mathbb{R}}$$
*and $d_{\Lambda^2}(\sigma, t) \neq 0$ for all $\sigma \neq 1/2$.*

*Proof.* Since $\|\hat{\xi}_{\Lambda^2}\|_{\ell^2} = 1$, there exists at least one Fourier mode $n_0 \in \mathbb{Z}$ such that $|\hat{\xi}_{n_0}^{(\Lambda^2)}| > 0$.
...
Since $\operatorname{Im}(d_{\Lambda^2}(\sigma, t)) = (\sigma - 1/2) \mathcal{S}_{\Lambda^2}(\sigma, t)$, its sign is strictly determined by $(\sigma - 1/2)$:
- $\sigma > 1/2 \implies \operatorname{Im}(d_{\Lambda^2}(\sigma, t)) > 0$
- $\sigma < 1/2 \implies \operatorname{Im}(d_{\Lambda^2}(\sigma, t)) < 0$
- $\operatorname{Im}(d_{\Lambda^2}(\sigma, t)) = 0 \iff \sigma = 1/2$.

For $d_{\Lambda^2}(\sigma, t) = 0$, we must simultaneously have $\operatorname{Re}(d_{\Lambda^2}) = 0$ and $\operatorname{Im}(d_{\Lambda^2}) = 0$. Since $\operatorname{Im}(d_{\Lambda^2}) \neq 0$ for all $\sigma \neq 1/2$, no roots exist off the critical line. $\blacksquare$
...
Let $H(t) = U \Lambda(t) U^*$ be the spectral diagonalization of $H(t)$ with real eigenvalues $\Lambda(t) = \operatorname{diag}(\mu_1(t), \dots, \mu_{2N}(t))$. Then:
...
1. The operator $A$ is closed and symmetric with deficiency indices:
   $$\operatorname{def}(A) = (\dim \operatorname{Ker}(A^* - i), \, \dim \operatorname{Ker}(A^* + i)) = (1, 1)$$
...
3. The spectrum $\operatorname{Spec}(A_\theta) \subset \mathbb{R}$ of every self-adjoint extension is purely real.
...
5. Since the spectrum of $A_\infty$ is strictly real ($\operatorname{Spec}(A_\infty) \subset \mathbb{R}$), no eigenvalues with non-zero imaginary part $\eta = \sigma - 1/2 \neq 0$ can exist.
...
| **Maximum Spectral Margin** | $\mathbf{+5.1755 \times 10^{-1}}$ | $> 0$ | **Validated** |
...
3. **Panel (C) — Secular Imaginary Sign Transition:** Maps $\operatorname{sgn}(\operatorname{Im} d) \cdot \log_{10}(|\operatorname{Im} d|)$. The sign flip across $\sigma = 1/2$ is sharp, with $\operatorname{Im}(d_{\Lambda^2}) > 0$ for $\sigma > 1/2$ (dark red) and $\operatorname{Im}(d_{\Lambda^2}) < 0$ for $\sigma < 1/2$ (dark blue), with $\operatorname{Im}(d) \equiv 0$ exclusively along $\sigma = 1/2$.
...
5. **Panel (E) — Critical Line Interlacing Secular Zeros:** Displays $\operatorname{Re} d_{\Lambda^2}(1/2, t)$ along the critical line. The clean zero crossings locate the automorphic non-trivial zeros $\gamma_k$.
6. **Panel (F) — Exterior Power Satake Trace Invariants:** Plots $\operatorname{Tr}(\Lambda^2(A_p))$ across primes $p \le 100$ comparing $\mathrm{Sym}^3(\Delta)$, $\mathrm{Sp}_4$, $\Delta \boxplus \Delta$, and generic $\mathrm{GL}_4$.
...
- **$\Lambda^2 \mathrm{GL}_{2n}$ and $\mathrm{SO}_{4n}$:** The general exterior square on $\mathrm{GL}_{2n}$ governs the functorial lifting from $\mathrm{Sp}_{2n}$ to $\mathrm{GL}_{2n}$, where $L(s, \Pi, \Lambda^2) = \zeta(s) L(s, \tau, \operatorname{std}_{\mathrm{SO}_{2n+1}})$.
