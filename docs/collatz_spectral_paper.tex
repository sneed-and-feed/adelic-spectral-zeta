...
$2^{1/2^{n-1}}$, establishing a precise directed spectral
``speed limit.''  Second, the limit $2^{1/2^{n-1}} \to 1$ as
...
\begin{enumerate}[(i)]
  \item The \emph{directed} Collatz tower has an exact, closed-form
    ...
    $2^{1/2^{n-1}}$, converging to the unit circle as $n \to \infty$.
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{\texttt{eigenvalue\_magnitude\_squared\_eq}}: combining
    ...
    $\abs{\lambda}^2 = 2^{1/2^{n-2}}$, i.e., $\abs{\lambda} = 2^{1/2^{n-1}}$.
\end{enumerate}

\begin{theorem}[Asymptotic gap collapse; \texttt{AsymptoticGap.lean}]
  \label{thm:asymp}
  \[
    \lim_{n \to \infty} 2^{1/2^{n-1}} = 1.
  \]
...
$1/2^{n-1} \to 0$ via \texttt{tendsto\_pow\_atTop\_nhds\_zero\_of\_lt\_one}
(half-powers).
...
\begin{table}[h!]
  ...
  \begin{tabular}{l l}
    ...
    \texttt{AsymptoticGap.lean} & Limit $2^{1/2^{n-1}} \to 1$ \\
    \hline
  \end{tabular}
...
\paragraph{P5 (Long-term). Adèlic limit and $L$-function zeros.}
The projective limit $\varprojlim G_d$ is the 2-adic Schreier graph.
Its spectral theory is related to the $p$-adic $L$-functions of the
Collatz generator.  Connecting this to the Adèlic Dirac operator
framework of the parent project is the long-term mathematical goal.
...
\begin{itemize}
  \item The exact eigenvalue magnitude $\abs{\lambda} = 2^{1/2^{n-1}}$
    for the directed Collatz twisted block.
  \item The limit $2^{1/2^{n-1}} \to 1$, formally collapsing the
...
collapses to zero.  Together these results constitute the first
machine-verified spectral analysis of the Collatz system and provide a
formal obstruction to rapid-mixing proofs of the Collatz conjecture.
