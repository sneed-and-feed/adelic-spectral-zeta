...
This is the standard Lasota-Yorke inequality with contraction coefficient $\theta = 2^{-\alpha} \lt 1$. $\square$
...
Since $2^{-\alpha} \lt 1$, taking $n \to \infty$ yields $v_\alpha(f) = 0$. Therefore, $f$ is constant. Since $B$ is Markov ($B 1 = 1$), any constant $f = c \cdot 1$ satisfies $Bf = f$, forcing $\lambda = 1$. $\square$
...
*Proof.* By Theorem 5, the essential spectral radius is bounded by $2^{-\alpha} \lt 1$. Therefore, any spectrum in the region $\{z \mid |z| \gt 2^{-\alpha}\}$ must consist of isolated eigenvalues of finite multiplicity.
By Lemma 6, the only eigenvalue on the unit circle is $1.0.$
Since the spectrum of a bounded operator is closed, and the only point of the spectrum on the unit circle is the isolated eigenvalue $1.0$, the remaining eigenvalues in the region $\{z \mid |z| \gt 2^{-\alpha}\}$ must be bounded away from the unit circle.
Thus, there exists some $r \lt 1$ such that all eigenvalues other than $1.0$ satisfy $|z| \le r$.
