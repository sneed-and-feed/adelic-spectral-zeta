...
Let $\mathbb{Q}_p$ be the field of $p$-adic numbers with absolute value $|x|_p = p^{-\operatorname{ord}_p(x)}$, ring of integers $\mathbb{Z}_p = \{x \in \mathbb{Q}_p : |x|_p \le 1\}$, and maximal ideal $\mathfrak{p} = p \mathbb{Z}_p$.
...
2. **Exterior Region ($|z - x|_p = p^{-m} > p^{-k}$ with $m < k$):** The boundary point lies outside the ball, branching off at scale $p^m$. Then:
...
- For any set of isometric boundary triples, the variation $\operatorname{std}(W_3 / F_{\text{conf}}) / \operatorname{mean}$ is **$0.00 \times 10^{-16}$** (exact invariance).
...
1. **Origin Value:** For all regular Satake parameters $z$ in the interior of the maximal torus, $\Phi_z^{G_2}(0, 0) = 1.00000000000000$ to machine precision (residual $< 10^{-14}$).
...
| :--- | :--- | :--- | :--- | :--- |
| **Witten Diagram $p=2$** | Relative std/mean across boundary triples | $0.00 \times 10^{-16}$ | $< 10^{-12}$ | **PASS** |
| **Witten Diagram $p=3$** | Relative std/mean across boundary triples | $0.00 \times 10^{-16}$ | $< 10^{-12}$ | **PASS** |
| **Witten Diagram $p=5$** | Relative std/mean across boundary triples | $0.00 \times 10^{-16}$ | $< 10^{-12}$ | **PASS** |
| **$G_2$ Weyl Group** | Group order $|W(G_2)|$ | $12$ | $= 12$ | **PASS** |
| **Macdonald $\Phi_z(0,0)$** | Residual at $z = (0.4, 0.7)$ | $1.53 \times 10^{-12}$ | $< 10^{-10}$ | **PASS** |
| **Macdonald $\Phi_z(0,0)$** | Residual at $z = (1.2, -0.8)$ | $3.80 \times 10^{-15}$ | $< 10^{-10}$ | **PASS** |
| **Macdonald $\Phi_z(0,0)$** | Residual at $z = (2.1, 1.5)$ | $8.72 \times 10^{-16}$ | $< 10^{-10}$ | **PASS** |
| **$G_2$ Torus Operator** | Regular Degree of $T_{\text{short}}$ and $T_{\text{long}}$ | $6, 6$ | $= 6$ | **PASS** |
| **$G_2$ Commutator** | Max Residual $\|[T_{\text{short}}, T_{\text{long}}]\|_\infty$ | $0.00 \times 10^{-16}$ | $< 10^{-15}$ | **PASS** |
| **$G_2$ Commutator** | Frobenius Norm $\|[T_{\text{short}}, T_{\text{long}}]\|_F$ | $0.00 \times 10^{-16}$ | $< 10^{-15}$ | **PASS** |
...
3. **Panel (c) — Macdonald Spherical Wavefunction $\operatorname{Re}(\Phi_z^{G_2})$ on $G_2$ Apartment:** 2D contour map of the joint eigenfunction across the hexagonal apartment lattice, showing the 6 short root arrows (green), 6 long root arrows (magenta), and the fundamental Weyl chamber $\mathcal{A}^+(G_2)$ (gold).
