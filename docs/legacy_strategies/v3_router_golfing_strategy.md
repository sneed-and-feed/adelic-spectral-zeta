...
3. **Roadmap Consistency:** The V3 roadmap explicitly denotes the polarized routing depths as $d_h = \mathbb{1}[z \gt 0]$. The subscript $h$ confirms that this decision was mathematically intended to be made per-head, not per-token.
...
Once training completes and the gates have polarized, the continuous framework is discarded. The routing depths are statically extracted as $d_h = \mathbb{1}[z_h \gt 0]$ and passed directly into the Triton block-sparse kernel for $O(N \log N)$ execution.
