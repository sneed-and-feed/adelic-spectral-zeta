...
inducing an exact spectrum tower $\operatorname{spec}(A_n) = \operatorname{spec}(A_{n-1}) \cup \operatorname{spec}(T_n)$. In the additive Fourier character basis on odd residues, $T_n$ is unitarily equivalent to two decoupled 1D tight-binding ring Hamiltonians $H_1 \oplus H_2$ of length $L = 2^{n-2}$, governed by quasi-periodic 3-adic hopping amplitudes $w_{k_j} = 1 + \exp(-2\pi i (3^j \bmod 2^n)/2^n)$. We prove that $T_n$ is strictly bipartite (forcing exact spectral inversion symmetry $\operatorname{spec}(T_n) = -\operatorname{spec}(T_n)$) and possesses exact Kramers-like double degeneracy across all eigenvalues. 

We connect this Hadamard decomposition to matrix-valued **Schur complement renormalization** $\mathcal{S}_n(z) = (zI - M_0) - M_1(zI - M_0)^{-1}M_1$, contrasting the arithmetic 3-adic carry propagation with classical 1D scalar spectral decimation in self-similar automaton groups (such as the Grigorchuk and Basilica groups). Analysis of the second eigenfunction $\psi_2$ reveals an Inverse Participation Ratio $\operatorname{IPR}(\psi_2) = \Theta(1/N)$, constant participation ratio $\operatorname{PR} \approx 0.36$, and information entropy ratio $S/\ln(N) \to 1$, establishing that low-lying excitations are macroscopic, delocalized fractal waves.
...
The associated directed relation matrix $D_n \in \operatorname{Mat}_{2^n \times 2^n}(\mathbb{R})$ has entries:
$$(D_n)_{x, y} = \begin{cases} 1 & \text{if } y \equiv 3x \pmod{2^n} \text{ or } y \equiv 3x - 1 \pmod{2^n}, \\ 0 & \text{otherwise.} \end{cases}$$
Because $3$ is coprime to $2^n$, the inverse map $y \mapsto 3^{-1}y \pmod{2^n}$ is well-defined and bijective. Hence, every vertex $x \in V_n$ has out-degree $\operatorname{deg}_{\text{out}}(x) = 2$ and in-degree $\operatorname{deg}_{\text{in}}(x) = 2$.

The **symmetrized undirected adjacency matrix** $A_n \in \operatorname{Mat}_{2^n \times 2^n}(\mathbb{R})$ is given by:
$$A_n = D_n + D_n^\top.$$
...
where $M_0, M_1 \in \operatorname{Mat}_{2^{n-1} \times 2^{n-1}}(\mathbb{R})$ are real symmetric matrices defined by:
...
*where $A_1 = \begin{pmatrix} 2 & 2 \\ 2 & 2 \end{pmatrix}$ has $\operatorname{spec}(A_1) = \{4, 0\}$.*
...
Within cycle $C_1$, let $|j\rangle = \chi_{3^j \bmod 2^n}$ for $j = 0, 1, \dots, L-1$. The operator $T_n = S_n + S_n^*$ restricted to $C_1$ is represented by the Hermitian 1D tight-binding ring Hamiltonian $H_1 \in \operatorname{Mat}_{L \times L}(\mathbb{C})$:
...
$$\operatorname{spec}(T_n) = -\operatorname{spec}(T_n).$$
*That is, if $\lambda \in \operatorname{spec}(T_n)$ with multiplicity $m$, then $-\lambda \in \operatorname{spec}(T_n)$ with multiplicity $m$.*
...
Thus, the Hamiltonian $H_2$ on cycle $C_2$ satisfies $H_2 = \overline{H_1} = H_1^\top$. Since $H_1$ is Hermitian, its eigenvalues are real, so $\operatorname{spec}(H_2) = \operatorname{spec}(\overline{H_1}) = \overline{\operatorname{spec}(H_1)} = \operatorname{spec}(H_1)$.
...
| Scale $n$ | Vertices $|V| = 2^n$ | Ring Dim $L = 2^{n-2}$ | $\lambda_1$ (Perron) | $\lambda_2$ | Gap $\Delta(A_n) = 4 - \lambda_2$ | $\lambda_{\max}(T_n)$ | $\operatorname{IPR}(\psi_2)$ | $\operatorname{PR} = \frac{1}{N \cdot \text{IPR}}$ | Entropy Ratio $S/S_{\max}$ | Solver Time (s) |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
- **Panel (c) (Bottom-Left):** Semi-log plot of Inverse Participation Ratio $\operatorname{IPR}(\psi_2) = \sum_x \psi_2(x)^4$ alongside Participation Ratio $\operatorname{PR} = 1 / (N \cdot \text{IPR})$. The IPR decays precisely as $\Theta(1/N)$, keeping $\operatorname{PR} \approx 0.36 \pm 0.04$ constant across all $n$.
...
- **Panel (a) (Left):** Scatter plot of eigenvalue towers $\operatorname{spec}(T_n)$ for levels $n=3, 4, 5, 6, 7$. Shows the recursive layering of discrete energy bands filling the interval $[-4, 4]$.
...
$$g = \sigma (g_0, g_1), \quad \sigma \in \operatorname{Sym}(\{0, 1\}), \; g_0, g_1 \in G.$$
...
3. **Recursive Hadamard Decomposition:** Proved $H_n A_n H_n = \operatorname{diag}(A_{n-1}, T_n)$, fully connecting the spectrum to the inductive tower $\operatorname{spec}(A_n) = \operatorname{spec}(A_{n-1}) \cup \operatorname{spec}(T_n)$.
...
5. **Exact Algebraic Symmetries:** Proved exact bipartite spectral inversion symmetry $\operatorname{spec}(T_n) = -\operatorname{spec}(T_n)$ and double multiplicity for all non-zero eigenvalues.
6. **Wavefunction Extendedness:** Demonstrated that the critical mode $\psi_2$ is fully delocalized with participation ratio $\operatorname{PR} \approx 0.36$ and entropy ratio $S/\ln(N) \to 1$.
