...
- **Dynamical Zeta Function:**
  $$\zeta_n(u) = \frac{1}{\det(I - u D_n)} = \exp\left( \sum_{m=1}^\infty \frac{u^m}{m} \operatorname{Tr}(D_n^m) \right)$$
- **Closed Geodesic Counting:**
  The trace $\operatorname{Tr}(D_n^m)$ counts the number of periodic points of period $m$. Using the Ihara-Bass formula formalized in `IharaBass.lean`, closed walks without backtracking can be enumerated analytically.
...
- **Exact Transition Kernels:**
  ...
  $$(D_n^t)_{x, y} = \frac{2^t}{2^n} + \sum_{k=2}^n \sum_{\lambda \in \operatorname{spec}(S_k)} \lambda^t \psi_\lambda(x) \overline{\phi_\lambda(y)}$$
- **Measure Concentration:**
  Because $|\lambda| = 2^{2^{-(k-1)}} \le \sqrt{2} < 2$ for all non-Perron eigenvalues, the non-equilibrium transient decays exponentially with a spectral gap of $2 - \sqrt{2}$.
