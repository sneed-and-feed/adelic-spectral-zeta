...
3. **For $m \lt j-1$ (so $|x-y|_2 \gt 2^{-j+1}$):**
...
For $|x-y|_2 \gt 2^{-M}$, the distance is bounded below, and the kernel is bounded:
...
$$
\|[D, f]\|_{HS}^2 = \iint_{\mathbb{Z}_2 \times \mathbb{Z}_2} |K(x, y)|^2 d\mu_2(x) d\mu_2(y) \le \iint_{|x-y|_2 \gt 2^{-M}} 4 \cdot 2^{4M} \|f\|_\infty^2 d\mu_2(x) d\mu_2(y) \lt \infty
$$
