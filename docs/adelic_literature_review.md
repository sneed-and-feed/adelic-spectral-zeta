...
*   **The Graph-Theoretic Riemann Hypothesis:** A $(d+1)$-regular graph is a Ramanujan graph *if and only if* its Ihara zeta function satisfies the analogue of the Riemann Hypothesis. Specifically, all poles of $Z(q^{-s})$ in the critical strip must lie on the "critical line" $\mathrm{Re}(s) = 1/2$.
