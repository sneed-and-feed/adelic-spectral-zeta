...
We conduct seven experiments that progressively build the case for dynamic ultrametric attention.
...
Dynamic ultrametric attention groks 2$\times$ faster than linear attention and crosses the 95\% threshold that dense attention never reaches. The converged gates reveal emergent layer specialization: Layer~0 $\to$ tree bias (hierarchical parsing), Layer~1 $\to$ dense (aggregation).
...
The model again discovered the same two-layer decomposition: Layer~0 parses the hierarchical tree structure; Layer~1 densely aggregates results.
...
\textbf{Self-organizing topologies.} The most striking finding is the \emph{consistency} of the emergent layer specialization. On both Dyck bracket matching and ListOps arithmetic, the model converges to the same motif: sparse hierarchical parsing in early layers, dense aggregation in later layers. This decomposition was never imposed---it emerged from gradient descent on the Gumbel-Sigmoid gates.
...
\textbf{Limitations.} (1)~No backward pass exists for the Triton kernel; training uses dense attention. (2)~The binary tree assumption may not capture variable branching factors in natural language. (3)~ListOps accuracy (62.7\%) has not fully grokked; longer training or larger models may be needed. (4)~Fixed block size ($B=128$) cannot capture sub-block sparsity patterns.
