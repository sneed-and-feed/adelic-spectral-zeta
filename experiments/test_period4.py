"""
Adelic Spectral Zeta: test_period4.py
"""
...
def main():
    for d in [4, 5, 6, 7, 8]:
        ...
        v_sin_pi_2 = np.array([math.sin(math.pi * x / 2) for x in range(N_half)])
        v_cos_pi_2 = np.array([math.cos(math.pi * x / 2) for x in range(N_half)])
        
        rq_sin = rayleigh_quotient(anti, v_sin_pi_2)
        rq_cos = rayleigh_quotient(anti, v_cos_pi_2)
        
        print(f"--- d = {d} ---")
        print(f"sym_2 = {lambda_sym_2:.4f}, anti_max = {anti_max:.4f}")
        print(f"RQ(sin_pi_2) = {rq_sin:.4f}")
        print(f"RQ(cos_pi_2) = {rq_cos:.4f}")
        
        if rq_sin > lambda_sym_2: print("WINNER: sin_pi_2")
        if rq_cos > lambda_sym_2: print("WINNER: cos_pi_2")
