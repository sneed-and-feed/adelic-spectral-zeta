...
WD = 0.1
STEPS = 15000
BATCH_SIZE = 8  # Reduced to avoid OOM on PyTorch Dense attention

def main():
    print("=" * 80)
    print("  PHASE 2: Scaling to 2048 Tokens")
    print("=" * 80)
    ...
    # Overwrite pos_embed to support massive sequence length
    model.pos_embed = nn.Embedding(SEQ_LEN, EMBED_DIM).to(DEVICE)

    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WD)
    
    print(f"\n[PHASE A] Training model for {STEPS} steps...")
    print("          (Warning: O(N^2) dense attention during training may be slow!)")
    model.train()
    for step in range(STEPS):
        ...
        if step % 500 == 0 or step == STEPS - 1:
            model.eval()
            with torch.no_grad():
                sub = min(128, len(test_seqs))  # Evaluated on smaller slice to save memory
                acc = (model(test_seqs[:sub]).argmax(-1) == test_labels[:sub]).float().mean().item()
            print(f"  Step {step:>5} | Loss: {loss.item():.4f} | Test Acc: {acc:.3f} | Tau: {tau:.2f}")
            model.train()
    ...
    with torch.no_grad():
        ...
        for i, layer in enumerate(model.layers):
            attn = layer['attn']
            dq = attn.depth_q(model.tok_embed(sample) + model.pos_embed(torch.arange(SEQ_LEN, device=DEVICE).unsqueeze(0).expand(8, SEQ_LEN)))
            dk = attn.depth_k(model.tok_embed(sample) + model.pos_embed(torch.arange(SEQ_LEN, device=DEVICE).unsqueeze(0).expand(8, SEQ_LEN)))
    ...
    # ---------------------------------------------------------
    print("\n[PHASE C] Inference Benchmarking at 2048 Tokens...")
    ...
    # Batch size 8 to avoid PyTorch Dense OOM at 2048 tokens
    batch_inf = 8
