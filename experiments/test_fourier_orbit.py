"""
Adelic Spectral Zeta: test_fourier_orbit.py
"""
...
def main():
    for d in [6, 7, 8]:
        ...
        L = N_half // 2  # Length of the orbit is 2^{d-3}
        
        # Test vector: sum_j cos(pi * j / L) cos(2pi * 3^j * x / N)
        v_test = np.zeros(N_half)
        for x in range(N_half):
            val = 0
            for j in range(L):
                freq = pow(3, j, N)
                val += math.cos(math.pi * j / L) * math.cos(2 * math.pi * freq * x / N)
            v_test[x] = val
            
        rq_cos = rayleigh_quotient(anti, v_test)
        
        # What about sin(pi * j / L)?
        v_test_sin = np.zeros(N_half)
        for x in range(N_half):
            val = 0
            for j in range(L):
                freq = pow(3, j, N)
                val += math.sin(math.pi * j / L) * math.cos(2 * math.pi * freq * x / N)
            v_test_sin[x] = val
            
        rq_sin = rayleigh_quotient(anti, v_test_sin)
        
        print(f"--- d = {d} ---")
        print(f"sym_2 = {lambda_sym_2:.4f}")
        print(f"RQ(cos) = {rq_cos:.4f}")
        print(f"RQ(sin) = {rq_sin:.4f}")
        
        if rq_cos > lambda_sym_2 or rq_sin > lambda_sym_2:
            print("WINNER: Fourier orbit test vector beats sym_2!")
