"""
Adelic Spectral Zeta: test_power_iteration.py
"""
...
def main():
    for d in [6, 7, 8, 9, 10]:
        ...
        # Initial guess: x mod 2
        v_parity = np.array([1 if x % 2 == 1 else -1 for x in range(N_half)])
        
        v = v_parity
        for i in range(10):
            rq = rayleigh_quotient(anti, v)
            if rq > lambda_sym_2:
                print(f"Iter {i}: WINNER! RQ = {rq:.4f}")
                break
            v = anti.dot(v)
        else:
            print(f"Parity failed. Best RQ = {rq:.4f}")
            
        # Initial guess: ones
        v_ones = np.ones(N_half)
        v = v_ones
        for i in range(10):
            rq = rayleigh_quotient(anti, v)
            if rq > lambda_sym_2:
                print(f"Iter {i} (ones): WINNER! RQ = {rq:.4f}")
                break
            v = anti.dot(v)
            if np.all(v == 0): break
        ...
        np.random.seed(42)
        v_rand = np.random.randint(-1, 2, size=N_half)
        v = v_rand
        for i in range(10):
            rq = rayleigh_quotient(anti, v)
            if rq > lambda_sym_2:
                print(f"Iter {i} (rand): WINNER! RQ = {rq:.4f}")
                break
            v = anti.dot(v)
