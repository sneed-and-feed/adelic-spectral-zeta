...
def main():
    for d in [6, 7, 8, 9]:
        ...
        # tests
        v_cos1 = np.array([math.cos(math.pi * x / N_half) for x in range(N_half)])
        v_sin1 = np.array([math.sin(math.pi * x / N_half) for x in range(N_half)])
        v_cos2 = np.array([math.cos(2 * math.pi * x / N_half) for x in range(N_half)])
        v_sin2 = np.array([math.sin(2 * math.pi * x / N_half) for x in range(N_half)])
