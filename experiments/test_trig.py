...
def main():
    for d in [4, 5, 6, 7]:
        ...
        for x in range(len(v)):
            # clip to [-1, 1] due to float inaccuracies
            val = max(min(v[x], 1.0), -1.0)
            angle = math.asin(val) / math.pi
            arcsins.append((x, angle))
            
        # print sorted by angle
        arcsins.sort(key=lambda x: x[1])
        for x, angle in arcsins:
            print(f"x={x:2d}: angle/pi = {angle:7.4f} (val = {v[x]:7.4f})")
