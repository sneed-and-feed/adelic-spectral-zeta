...
def finToZMod {n : ℕ} (x : Fin (n + 1)) : ZMod (n + 1) := x

lemma charmatrix_cyclic_submatrix_00 (n : ℕ) (W : ZMod (n + 2) → R) :
    (charmatrix (cyclicWeightMatrix W)).submatrix (fun i => finToZMod (Fin.succ i)) (fun j => finToZMod (Fin.succ j)) =
      charmatrix (shiftMatrix (n + 1) (fun x => W (finToZMod (Fin.succ x)))) := by
  ext i j n_coeff
  dsimp [charmatrix, Matrix.submatrix_apply, Matrix.sub_apply, Matrix.diagonal_apply, shiftMatrix, cyclicWeightMatrix, finToZMod]
...
lemma charmatrix_cyclic_submatrix_0n (n : ℕ) (W : ZMod (n + 2) → R) :
    (charmatrix (cyclicWeightMatrix W)).submatrix (fun i => finToZMod (Fin.succ i)) (fun j => finToZMod ((Fin.last (n + 1)).succAbove j)) =
      upperBidiagonal (n + 1) (fun x => W (finToZMod (Fin.castSucc x))) := by
  ext i j n_coeff
  dsimp [charmatrix, Matrix.submatrix_apply, Matrix.sub_apply, Matrix.diagonal_apply, cyclicWeightMatrix, upperBidiagonal, finToZMod]
  simp only [Fin.succAbove_last]
  have eq1 : (Fin.succ i : Fin (n + 2)) = (Fin.castSucc j : Fin (n + 2)) ↔ (i : ℕ) + 1 = (j : ℕ) := by
    constructor
    · intro h
      have h_val := congrArg Fin.val h
    ...
    · intro h
      ...
      change (i : ℕ) + 1 = (j : ℕ)
      omega
  have eq2 : (Fin.succ i : Fin (n + 2)) = (Fin.castSucc j : Fin (n + 2)) + 1 ↔ (i : ℕ) = (j : ℕ) := by
    constructor
    · intro h
      have h_val := congrArg Fin.val h
...
theorem charpoly_cyclicWeightMatrix :
  ...
  | succ m =>
    cases m with
    | zero =>
      simp only [Matrix.charpoly, Matrix.det_fin_one, charmatrix, Matrix.sub_apply, Matrix.diagonal_apply,
        Matrix.map_apply, cyclicWeightMatrix, Fin.ext_iff]
      have h1 : (0 : ℕ) = (0 : ℕ) + 1 ↔ False := by exact iff_false_intro (by omega)
      have h2 : (0 : ZMod 1) = (0 : ZMod 1) + 1 ↔ True := iff_true_intro (Subsingleton.elim _ _)
      simp [h1, h2, pow_one]
    | succ n =>
      ...
      · change (-1) ^ (0 : ℕ) * _ * ((charmatrix (Matrix.of fun (i j : Fin (n + 2)) => cyclicWeightMatrix W i j)).submatrix Fin.succ Fin.succ).det + _ = _
        ...
        have h_shift := charpoly_shiftMatrix (n + 1) (fun x => W (finToZMod (Fin.succ x)))
        ...
        rw [h_pow]
        have h_eq1 : ∀ i : Fin (n+2), (Matrix.of fun (i j : Fin (n + 2)) => cyclicWeightMatrix W i j) 0 i = cyclicWeightMatrix W 0 i := fun _ => rfl
        simp only [h_eq1]
        dsimp [cyclicWeightMatrix]
        have c1 : (0 : Fin (n + 2)) = Fin.last (n + 1) ↔ False := by
          intro h
          have h' : (0 : ℕ) = n + 1 := congrArg Fin.val h
          omega
        have c2 : (0 : ℕ) = (Fin.last (n + 1) : ℕ) + 1 ↔ False := by omega
        simp only [c1, c2, if_false, sub_zero, zero_sub, Fin.prod_univ_succ, zero_add, mul_neg, mul_zero]
        ring
      · exact mem_univ 0
      ...
      · intro h
        ...
        omega
      · intro b _ hb0 hbn
        have hb0' : (b : ℕ) = 0 ↔ False := by apply iff_false_intro; intro h; apply hb0; exact Fin.ext h.symm
        have hbn' : (b : ℕ) = n + 1 ↔ False := by apply iff_false_intro; intro h; apply hbn; exact Fin.ext h.symm
        have h_eq1 : (Matrix.of fun (i j : Fin (n + 2)) => cyclicWeightMatrix W i j) 0 b = cyclicWeightMatrix W 0 b := rfl
        simp only [h_eq1]
        dsimp [cyclicWeightMatrix]
        have c2 : (0 : Fin (n + 2)) = b + 1 ↔ False := by
          apply iff_false_intro
          intro h
          have h1 : (b : ℕ) + 1 < n + 2 := by
            have : (b : ℕ) < n + 2 := b.is_lt
            omega
