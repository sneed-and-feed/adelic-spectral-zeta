...
lemma neg_val_odd {n : ℕ} {a : ZMod (2^n)} (hn : 1 ≤ n) (ha : Odd a.val) : Odd (-a).val := by
  ...
  have h_ne : a ≠ 0 := by rintro rfl; exact even_zero.not_odd ha
  rw [ZMod.val_neg_of_ne_zero h_ne]
  exact (Even.pow_of_ne_zero even_two (by omega)).sub_odd (ZMod.val_lt a).le ha
...
lemma neg_mem_primitiveRoots (μ : F) (hn : 2 ≤ n) (hμ : μ ∈ primitiveRoots (2^n) F) :
  ...
  have heven : Even (2^n) := Even.pow_of_ne_zero even_two (by omega)
  refine ⟨by rw [neg_pow, heven.neg_one_pow, one_mul, hμ.pow_eq_one], fun l hl ↦ ?_⟩
  have heven_l : Even l := by
    have hdvd : 2^n ∣ 2 * l := hμ.dvd_of_pow_eq_one _ (by rw [pow_mul', neg_sq, ←pow_mul', hl, one_pow])
...
lemma W_1_mul_W_2_eq_two (hn : 2 ≤ n) (C_1 C_2 : Finset (ZMod (2^n))) 
  ...
  have h_bij : ∏ x ∈ {x : ZMod (2^n) | Odd x.val}.toFinset, (1 + zeta ^ (-x).val) = 
    ...
    · simp only [Set.mem_toFinset, Set.mem_setOf_eq]
      intro a ha
      exact neg_mem_primitiveRoots zeta hn _ ((mem_primitiveRoots (by positivity)).mpr (hzeta.pow_of_coprime _ (odd_coprime_pow_two (neg_val_odd (by omega) ha))))
    ...
    · intro μ hμ
      have h_prim_neg_mu := (mem_primitiveRoots (by positivity)).mp (neg_mem_primitiveRoots zeta hn _ hμ)
      ...
      refine ⟨- (i : ZMod (2^n)), ?_, by rw [neg_neg, ZMod.val_natCast_of_lt hi_lt, hi_eq]⟩
      refine neg_val_odd (by omega) ?_
