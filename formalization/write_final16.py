...
lemma charmatrix_shiftMatrix_submatrix (n : ℕ) (W : Fin (n + 1) → R) :
  ...
  · have h_eq_nat : (i : ℕ) = (j : ℕ) := congrArg Fin.val h_eq
    ...
    have c2 : (Fin.succ i : ℕ) ≠ (Fin.succ j : ℕ) + 1 := by
      have hi : (Fin.succ i : ℕ) = (i : ℕ) + 1 := Fin.val_succ i
      have hj : (Fin.succ j : ℕ) = (j : ℕ) + 1 := Fin.val_succ j
      omega
    ...
    | succ n =>
      ...
      · have h_succ : (Fin.succAbove (0 : Fin (n+2)) : Fin (n + 1) → Fin (n + 2)) = Fin.succ := rfl
        ...
        have h_prod_neg : (∏ i : Fin n, -C (W (Fin.castSucc (Fin.succ i)))) = (-1 : Polynomial R) ^ n * ∏ i : Fin n, C (W (Fin.castSucc (Fin.succ i))) := by
          ...
          have hc : (Finset.univ : Finset (Fin n)).card = n := Finset.card_univ
          rw [← hc]
          have h_const : (∏ i : Fin n, (-1 : Polynomial R)) = (-1 : Polynomial R) ^ (Finset.univ : Finset (Fin n)).card := Finset.prod_const (-1 : Polynomial R)
          rw [h_const]
        rw [h_prod_neg]
        have h_pow_even : (-1 : Polynomial R) ^ (2 * n) = 1 := by
          rw [mul_comm, pow_mul]
        ...
        have h_pow_simp : (-1 : Polynomial R) ^ n * (-1) ^ n = 1 := by
          ...
          rw [this, h_pow_even]
        have h_final : -(X * (X ^ n * C 0)) + (X ^ 2 * X ^ n - C (W (Fin.last (n + 1))) * C (W (Fin.castSucc 0)) * ((-1 : Polynomial R) ^ n * ∏ i : Fin n, C (W (Fin.castSucc (Fin.succ i)))) * (-1 : Polynomial R) ^ n) = X ^ 2 * X ^ n - C (W 0 * W (Fin.last (n + 1)) * ∏ i : Fin n, W (Fin.castSucc (Fin.succ i))) := by
          have h_W0 : W (Fin.castSucc 0) = W 0 := rfl
          rw [h_W0]
          have h_rearrange : C (W (Fin.last (n + 1))) * C (W 0) * ((-1 : Polynomial R) ^ n * ∏ i : Fin n, C (W (Fin.castSucc (Fin.succ i)))) * (-1 : Polynomial R) ^ n = ((-1 : Polynomial R) ^ n * (-1) ^ n) * (C (W 0) * C (W (Fin.last (n + 1))) * ∏ i : Fin n, C (W (Fin.castSucc (Fin.succ i)))) := by ring
          rw [h_rearrange, h_pow_simp, one_mul]
          have h_C_mul : C (W 0) * C (W (Fin.last (n + 1))) * ∏ i : Fin n, C (W (Fin.castSucc (Fin.succ i))) = C (W 0 * W (Fin.last (n + 1)) * ∏ i : Fin n, W (Fin.castSucc (Fin.succ i))) := by
            simp only [map_mul]
            congr 1
            exact map_prod C _ _
          rw [h_C_mul]
          ring
        exact h_final
      · exact mem_univ 0
