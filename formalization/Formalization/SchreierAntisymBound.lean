...
def max_antisym_eig_lower_bound_hypothesis (d : ℕ) (hd : d ≥ 7) : Prop :=
    chain_rayleigh_quotient d ≤ max_antisym_eig hd
...
def chain_rayleigh_beats_prev_gap_hypothesis (d : ℕ) (hd : d ≥ 7) : Prop :=
    max_antisym_eig (by omega : d - 1 ≥ 3) < chain_rayleigh_quotient d
...
theorem relative_spectral_gap (d : ℕ) (hd : d ≥ 7)
    (h_beats : chain_rayleigh_beats_prev_gap_hypothesis d hd)
  ...
  calc max_antisym_eig (by omega : d - 1 ≥ 3)
    _ < chain_rayleigh_quotient d := h_beats
    _ ≤ max_antisym_eig hd := h_lower
