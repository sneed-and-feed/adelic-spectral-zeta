...
lemma prod_one_sub_primitive_roots (hzeta : IsPrimitiveRoot zeta (2^n)) :
    ∏ μ ∈ primitiveRoots (2^n) F, (1 - μ) = eval 1 (cyclotomic (2^n) F) := by
  rw [cyclotomic_eq_prod_X_sub_primitiveRoots hzeta, eval_prod]
  simp
...
lemma neg_val_odd {n : ℕ} {a : ZMod (2^n)} (hn : 1 ≤ n) (ha : Odd a.val) : Odd (-a).val := by
  ...
  have h_ne : a ≠ 0 := by
    rintro rfl
    rw [ZMod.val_zero] at ha
    exact Nat.not_odd_zero ha
...
-/
lemma W_1_mul_W_2_eq_two (hzeta : IsPrimitiveRoot zeta (2^n)) (hn : 2 ≤ n) (C_1 C_2 : Finset (ZMod (2^n)))
  ...
  have h_bij : ∏ x ∈ {x : ZMod (2^n) | Odd x.val}.toFinset, (1 + zeta ^ (-x).val) =
    ...
    · simp only [Set.mem_toFinset, Set.mem_ofPred_eq, neg_inj]
      intro a _ b _ heq
    ...
    · intro μ hμ
      ...
      obtain ⟨i, hi_lt, hi_eq⟩ := hzeta.eq_pow_of_pow_eq_one h_prim_neg_mu.pow_eq_one
      ...
      have hd : Odd (i : ZMod (2^n)).val := by
        rw [ZMod.val_natCast_of_lt hi_lt]
        by_contra h_not_odd
        have heven : 2 ∣ i := by
        ...
        have h2 : 2 ∣ Nat.gcd i (2^n) := Nat.dvd_gcd heven (dvd_pow_self 2 (by omega))
        rw [hi_coprime] at h2
        revert h2
        decide
      rw [Set.mem_toFinset, Set.mem_ofPred_eq]
...
-/
lemma eigenvalue_magnitude_squared_eq (hzeta : IsPrimitiveRoot zeta (2^n)) (lambda lambda_conj : F)
