...
theorem fredholm_eval_zero (n : ℕ) :
  ...
  dsimp [fredholmFactorization, perronFactor, baseTwistedFactor, cyclotomicTowerProd]
  simp only [eval_mul, eval_sub, eval_one, eval_X, eval_pow, sub_zero, mul_one,
             eval_prod, zero_pow (by omega : 2 ≠ 0), mul_zero]
...
lemma sum_two_pow_Icc (n : ℕ) (hn : 2 ≤ n) :
  ...
  | base =>
    ...
    rfl
  | succ m hm ih =>
    have h_le : 3 ≤ m + 1 := by omega
    rw [sum_Icc_succ_top h_le, ih]
    have h_term : 2 ^ (m + 1 - 1) = 2 ^ m := rfl
    have h_pow : 2 ^ (m + 1) = 2 ^ m + 2 ^ m := by ring
    have h4 : 4 ≤ 2 ^ m := by
      change 2^2 ≤ 2^m
      exact Nat.pow_le_pow_right (by omega) hm
    rw [h_term, h_pow, Nat.sub_add_comm h4]
