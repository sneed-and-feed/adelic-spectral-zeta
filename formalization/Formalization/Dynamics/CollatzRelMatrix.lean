...
lemma projDir_natCast {n : ℕ} (hn : n ≥ 1) (k : ℕ) : projDir n hn (k : ZMod (2^n)) = (k : ZMod (2^(n-1))) :=
  map_natCast _ _
...
lemma half_ne_zero {n : ℕ} (hn : n ≥ 2) : ((2^(n-1) : ℕ) : ZMod (2^n)) ≠ 0 := by
  intro contra
  have hdvd : 2^n ∣ 2^(n-1) := (CharP.cast_eq_zero_iff (ZMod (2^n)) (2^n) (2^(n-1))).mp contra
...
lemma zero_ne_neg_one {n : ℕ} (hn : n ≥ 2) : (0 : ZMod (2^(n-1))) ≠ -1 := by
  ...
  have : (1 : ZMod (2^(n-1))) = 0 := by
    calc (1 : ZMod (2^(n-1))) = 1 + 0 := by ring
      _ = 1 + -1 := by rw [← contra]
      _ = 0 := by ring
  have h_one_cast : ((1 : ℕ) : ZMod (2^(n-1))) = 0 := by exact_mod_cast this
  have hdvd : 2^(n-1) ∣ 1 := (CharP.cast_eq_zero_iff (ZMod (2^(n-1))) (2^(n-1)) 1).mp h_one_cast
...
lemma projDir_eq_iff {n : ℕ} (hn : n ≥ 2) (x y : ZMod (2^n)) :
  ...
  · intro h
    ...
    have h_cast : (((x - y).val : ℕ) : ZMod (2^(n-1))) = 0 := by
      have h_proj := h1
      rw [h_z] at h_proj
      rw [projDir_natCast (by omega)] at h_proj
      exact h_proj
    have h3 : 2^(n-1) ∣ (x - y).val :=
      (CharP.cast_eq_zero_iff (ZMod (2^(n-1))) (2^(n-1)) (x - y).val).mp h_cast
