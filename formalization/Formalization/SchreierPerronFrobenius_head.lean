...
lemma allpos_eigenvector_unique
    {B : Matrix n n ℝ} (hB : ∀ i j, 0 < B i j)
    (μ : ℝ) (v w : n → ℝ) (hv_pos : ∀ i, 0 < v i) (hw_pos : ∀ i, 0 < w i)
    (hv_eig : B.mulVec v = μ • v) (hw_eig : B.mulVec w = μ • w) :
    ∃ c : ℝ, w = c • v := by
  ...
  have ht_pos : 0 < t := by
    ...
    intro i _
    exact div_pos (hw_pos i) (hv_pos i)
  have h_diff_nn : ∀ i, 0 ≤ (w - t • v) i := by
    intro i
    simp only [Pi.sub_apply, Pi.smul_apply, smul_eq_mul]
    have hi : t ≤ ratios i := Finset.inf'_le ratios (Finset.mem_univ i)
    rw [sub_nonneg]
    rwa [le_div_iff (hv_pos i)] at hi
  have h_diff_eig : B.mulVec (w - t • v) = μ • (w - t • v) := by
    rw [mulVec_sub, mulVec_smul, hw_eig, hv_eig, smul_sub, smul_comm]
  by_cases h_diff_zero : w - t • v = 0
  · exact sub_eq_zero.mp h_diff_zero
  · exfalso
    have h_diff_pos_some : ∃ j, 0 < (w - t • v) j := by
      by_contra h_all
      push_neg at h_all
      apply h_diff_zero
      ext i
      exact le_antisymm (h_all i) (h_diff_nn i)
    have h_B_diff_pos := Matrix.mulVec_pos_of_allPos hB h_diff_nn h_diff_pos_some
    ...
    have : B.mulVec (w - t • v) i₀ = 0 := by rw [h_diff_eig]; simp [h_min_achieved]
...
private lemma mu_B_le_max_eig' {n : Type*} [Fintype n] [DecidableEq n] [Nonempty n]
  ...
  have h_dot : ∀ k, (μ_B - 1) * c k = eig k * c k := by
    ...
    have step2 : dotProduct (evec k : n → ℝ) (A.mulVec v_B) =
      ...
      rw [hA_herm.mulVec_eigenvectorBasis k]
