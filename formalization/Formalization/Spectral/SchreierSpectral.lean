...
lemma val_pi {d : ℕ} (_hd : d ≥ 3) (x : ZMod (2^(d-1))) :
  ...
  nth_rw 1 [h1]
  rw [pi_natCast]
  exact @ZMod.val_natCast (2^(d-2)) x.val
...
lemma A'_block_diag {d : ℕ} (hd : d ≥ 3) :
  ...
  · dsimp [A'_block_diag_target, weightedMatrix, sheetDiffMatrix, A'_matrix]
...
lemma s_card_eq_double {d : ℕ} (hd : d ≥ 3) (u v : ZMod (2^(d-2))) :
  ...
  have h_sum_prod : (∑ p : ZMod (2^(d-1)) × ZMod (2^(d-1)), if pi p.1 = v ∧ pi p.2 = u ∧ (G_d d).Adj p.1 p.2 then (1:ℚ) else 0) =
    ∑ p₁ : ZMod (2^(d-1)), ∑ p₂ : ZMod (2^(d-1)), if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0 := Fintype.sum_prod_type _
  rw [h_sum_prod]
  ...
  have h_sum_subset_x : (∑ p₁ : ZMod (2^(d-1)), ∑ p₂ : ZMod (2^(d-1)), if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0) =
      ∑ p₁ ∈ S_x, ∑ p₂ : ZMod (2^(d-1)), if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0 := by
    symm
  ...
  have h_sum_subset_y : ∀ p₁, (∑ p₂ : ZMod (2^(d-1)), if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0) =
      ∑ p₂ ∈ S_y, if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0 := by
    intro p₁
    symm
  ...
  rw [h_sum_subset_x]
  have h_sum_subset_y_sum : (∑ p₁ ∈ S_x, ∑ p₂ : ZMod (2^(d-1)), if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0) =
      ∑ p₁ ∈ S_x, ∑ p₂ ∈ S_y, if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0 := by
  ...
  have h_simplify_sum : (∑ p₁ ∈ S_x, ∑ p₂ ∈ S_y, if pi p₁ = v ∧ pi p₂ = u ∧ (G_d d).Adj p₁ p₂ then (1:ℚ) else 0) =
      ∑ p₁ ∈ S_x, ∑ p₂ ∈ S_y, if (G_d d).Adj p₁ p₂ then (1:ℚ) else 0 := by
    apply Finset.sum_congr rfl
    intro p₁ hp₁
    apply Finset.sum_congr rfl
    intro p₂ hp₂
  ...
  have h_eval : (∑ p₁ ∈ S_x, ∑ p₂ ∈ S_y, if (G_d d).Adj p₁ p₂ then (1:ℚ) else 0) = 
  ...
  · constructor
    ...
    · -- S_inv * A * S = block diag
      have hA : @adjacencyMatrix d = Matrix.reindex e.symm e.symm (A'_matrix hd) := by
        ...
        have h_ei : (sheetSplit hd).symm (e i) = i := (sheetSplit hd).left_inv i
        have h_ej : (sheetSplit hd).symm (e j) = j := (sheetSplit hd).left_inv j
        rw [h_ei, h_ej]
      rw [hA]
...
theorem charpoly_similarity {α : Type*} [CommRing α] {n : Type*} [Fintype n] [DecidableEq n]
  ...
  rw [h_one] at h_map
  have h_map_comm : F S * F S_inv = 1 := by rw [mul_eq_one_comm, h_map]
  unfold Matrix.charpoly
...
lemma A'_block_diag_target_eq_blockDiagMatrix {d : ℕ} (hd : d ≥ 3) :
  ...
  fin_cases b1 <;> fin_cases b2 <;>
    (dsimp [A'_block_diag_target, blockDiagMatrix, Matrix.fromBlocks, sumProdEquiv, Matrix.reindex_apply, Matrix.submatrix_apply]; rfl)
...
theorem realWeightedMatrix_isHermitian {d : ℕ} (hd : d ≥ 3) :
  ...
  have h1 : A'_matrix hd (j, 0) (i, 0) = A'_matrix hd (i, 0) (j, 0) := by
    ...
    have h_symm : (G_d d).Adj ((sheetSplit hd).symm (j, 0)) ((sheetSplit hd).symm (i, 0)) ↔ (G_d d).Adj ((sheetSplit hd).symm (i, 0)) ((sheetSplit hd).symm (j, 0)) := ⟨fun h => (G_d d).adj_symm h, fun h => (G_d d).adj_symm h⟩
  ...
  have h2 : A'_matrix hd (j, 0) (i, 1) = A'_matrix hd (i, 0) (j, 1) := by
    ...
    simp only [Matrix.reindex_apply, Equiv.symm_symm]
    dsimp [adjacencyMatrix]
    have h_symm : (G_d d).Adj ((sheetSplit hd).symm (j, 1)) ((sheetSplit hd).symm (i, 0)) ↔ (G_d d).Adj ((sheetSplit hd).symm (i, 0)) ((sheetSplit hd).symm (j, 1)) := ⟨fun h => (G_d d).adj_symm h, fun h => (G_d d).adj_symm h⟩
...
lemma hasEigenvalue_eigenvalues {n : Type*} [Fintype n] [DecidableEq n] {A : Matrix n n ℝ}
  ...
  have h_mem : (hA.eigenvectorBasis j : n → ℝ) ∈ Module.End.eigenspace (Matrix.toLin' A) (hA.eigenvalues j) := by
    rw [Module.End.mem_eigenspace_iff, toLin'_apply]
    exact hA.mulVec_eigenvectorBasis j
  have h_ne : (hA.eigenvectorBasis j : n → ℝ) ≠ 0 := by
    intro h
    have h_zero : hA.eigenvectorBasis.toBasis j = 0 := by
      ext k
      exact congr_fun h k
    exact hA.eigenvectorBasis.toBasis.ne_zero j h_zero
  exact Module.End.hasEigenvalue_of_hasEigenvector ⟨h_mem, h_ne⟩
...
lemma realSheetDiffMatrix_row_sum_le {d : ℕ} (hd : d ≥ 3) (u : ZMod (2^(d-2))) :
  ...
  have h_le : ∀ v, ‖realSheetDiffMatrix hd u v‖ ≤ algebraMap ℚ ℝ (A'_matrix hd (u, 0) (v, 0) + A'_matrix hd (u, 0) (v, 1)) := by
    intro v
    dsimp [realSheetDiffMatrix, sheetDiffMatrix, Matrix.map_apply]
    rw [map_sub, map_add]
...
theorem realAdjacencyMatrix_isHermitian {d : ℕ} :
  ...
  apply congrArg (algebraMap ℚ ℝ)
  have h_symm : (G_d d).Adj i j ↔ (G_d d).Adj j i := ⟨fun h => (G_d d).adj_symm h, fun h => (G_d d).adj_symm h⟩
