...
lemma pow_mod_eq_pow (hzeta : IsPrimitiveRoot zeta (2^n)) (x : ℕ) : zeta ^ (x % 2^n) = zeta ^ x := by
...
lemma pow_val_add (hzeta : IsPrimitiveRoot zeta (2^n)) (a b : ZMod (2^n)) : zeta ^ (a + b).val = zeta ^ a.val * zeta ^ b.val := by
...
-/
lemma D_n_chi (hzeta : IsPrimitiveRoot zeta (2^n)) (k : ZMod (2^n)) :
