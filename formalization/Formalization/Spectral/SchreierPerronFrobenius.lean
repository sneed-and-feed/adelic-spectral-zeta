...
lemma isIrreducible_of_connected {n : Type _} [Fintype n] [DecidableEq n] [Nonempty n]
    ...
    Matrix.IsPerronIrreducible (A + 1) := by
  refine ⟨fun i j => add_nonneg (hA_nn i j) (by simp only [Matrix.one_apply]; split_ifs <;> norm_num), ?_, ?_⟩
  · intro i j
    obtain ⟨w⟩ := h_conn.preconnected i j
    by_cases hij : i = j
    · use 1
      refine ⟨by norm_num, ?_⟩
      rw [hij, pow_one, Matrix.add_apply, Matrix.one_apply_eq]
  ...
  · let i := Classical.arbitrary n
    use i
    have : ((A + 1) : Matrix _ _ ℝ) i i = A i i + 1 := by rw [Matrix.add_apply, Matrix.one_apply_eq]
    rw [this]
...
lemma isPerronFrobeniusMax_of_connected {n : Type _} [Fintype n] [DecidableEq n] [Nonempty n]
  ...
  have hB_symm : ∀ i j, B i j = B j i := by
    ...
    rw [Matrix.add_apply, Matrix.add_apply, symm_of_herm hA_herm i j]
    by_cases h : i = j
    · rw [h]
    · rw [Matrix.one_apply_ne h, Matrix.one_apply_ne (Ne.symm h)]
  have hB_nn : ∀ i j, 0 ≤ B i j := fun i j => add_nonneg (hA_nn i j) (by simp only [Matrix.one_apply]; split_ifs <;> norm_num)
  ...
  · intro hj_eq
    ...
    have hw_i_neq : w_i ≠ 0 := by
      intro h
      have hevec_zero : evec i_max = 0 := by ext k; exact congr_fun h k
      have h1 : ‖evec i_max‖ = 0 := by rw [hevec_zero, norm_zero]
      have h2 : ‖evec i_max‖ = 1 := (OrthonormalBasis.orthonormal evec).1 i_max
      linarith
    have hw_j_neq : w_j ≠ 0 := by
      intro h
      have hevec_zero : evec j = 0 := by ext k; exact congr_fun h k
      have h1 : ‖evec j‖ = 0 := by rw [hevec_zero, norm_zero]
      have h2 : ‖evec j‖ = 1 := (OrthonormalBasis.orthonormal evec).1 j
      linarith
    ...
    have h_eq_vec : (evec i_max : EuclideanSpace ℝ n) = (ci / cj) • (evec j : EuclideanSpace ℝ n) := by
    ...
    have h_ne : i_max ≠ j := fun heq => h_neq heq.symm
    have h1 : evec.repr (evec i_max) i_max = 1 := by simp
    have h2 : evec.repr (evec j) i_max = 0 := by simp [h_ne]
    have h3 : (evec.repr ((ci / cj) • evec j)).ofLp i_max = 0 := by
      rw [map_smul]
      dsimp
      rw [h2, mul_zero]
    rw [h_eq_vec] at h1
