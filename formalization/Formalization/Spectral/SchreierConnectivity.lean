...
lemma pi_mul_three {d : ℕ} (x : ZMod (2^(d-1))) : pi (3 * x) = 3 * pi x := by
  have : (3 : ZMod (2^(d-1))) = ((3 : ℕ) : ZMod (2^(d-1))) := by norm_cast
  have h_pi3 : pi (3 : ZMod (2^(d-1))) = 3 := by rw [this, pi_natCast]; norm_cast
  rw [pi_mul, h_pi3]

lemma pi_sub_one {d : ℕ} (x : ZMod (2^(d-1))) : pi (x - 1) = pi x - 1 := by
  have : (1 : ZMod (2^(d-1))) = ((1 : ℕ) : ZMod (2^(d-1))) := by norm_cast
  have h_pi1 : pi (1 : ZMod (2^(d-1))) = 1 := by rw [this, pi_natCast]; norm_cast
  rw [pi_sub, h_pi1]
...
lemma G_d_degree_le {d : ℕ} (hd : d ≥ 3) (x : ZMod (2^(d-1))) :
  ...
  have h3 : ({3 * x, 3 * x - 1, inv3 hd * x, inv3 hd * (x + 1)} : Finset (ZMod (2^(d-1)))).card ≤ 4 := by
    linarith [Finset.card_insert_le (3*x) {3*x-1, inv3 hd*x, inv3 hd*(x+1)},
              Finset.card_insert_le (3*x-1) {inv3 hd*x, inv3 hd*(x+1)},
              Finset.card_insert_le (inv3 hd*x) {inv3 hd*(x+1)},
              Finset.card_singleton (inv3 hd*(x+1))]
  exact h2.trans h3
  ...
  have h_2x : 2 * x = 1 := by
    calc 2 * x = 3 * x - x := by ring
         _ = 1 := by 
           ...
           rw [this]
           ring
  have h_unit : IsUnit (2 : ZMod (2^(d-1))) := ⟨⟨2, x, h_2x, by rw [mul_comm, h_2x]⟩, rfl⟩
  ...
  · -- Case 4: pi x = 3 * v - 1
    ...
    · have h1 : pi (inv3 hd * (x + 1)) = pi (inv3 hd) * pi (x + 1) := pi_mul _ _
      have h_pi_add : pi (x + 1) = pi x + 1 := by
        have h_one : pi (1 : ZMod (2^(d-1))) = 1 := by
          have h1 : (1 : ZMod (2^(d-1))) = ((1 : ℕ) : ZMod (2^(d-1))) := by norm_cast
          rw [h1, pi_natCast]
          norm_cast
        rw [pi_add, h_one]
      rw [h1, h_pi_add, h]
