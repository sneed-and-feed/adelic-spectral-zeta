...
noncomputable def thermalNoiseLattice (N : ℕ) (p : ℝ≥0) (hp : p ≤ 1) : 
  ...
  PMF.ofFintype (fun e => ∏ k : QubitLattice N, (thermalNoiseSingle p hp (e k) : ENNReal)) (by
    sorry)
...
theorem adelic_code_threshold_exists :
  ...
  ∀ (p : ℝ≥0) (hp : p < p_c),
    ∃ (D : ∀ N, Decoder N),
      Filter.Tendsto
        (fun N => (thermalNoiseLattice N p (by sorry)).toOuterMeasure {e | isLogicalFailure N e (D N (syndromeMap N e))})
        Filter.atTop
        (𝓝 0) := by
  sorry
