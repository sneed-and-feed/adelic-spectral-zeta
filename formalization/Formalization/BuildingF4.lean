...
def radialLaplacianF4 (q : R) (f : ApartmentSiteF4 → R) : ApartmentSiteF4 → R :=
  fun v => radialT_short q f v + radialT_long q f v - regularDegreeF4 q * f v

theorem radialT_short_add (q : R) (f g : ApartmentSiteF4 → R) :
...
theorem radialT_long_add (q : R) (f g : ApartmentSiteF4 → R) :
...
theorem radialT_short_smul (q : R) (c : R) (f : ApartmentSiteF4 → R) :
    radialT_short q (fun v => c * f v) = fun v => c * radialT_short q f v := by
  ext ⟨x1, x2, x3, x4⟩
  dsimp [radialT_short, radialT_s_unit, radialT_s_diag_pos, radialT_s_diag_neg]
  ring

theorem radialT_long_smul (q : R) (c : R) (f : ApartmentSiteF4 → R) :
    radialT_long q (fun v => c * f v) = fun v => c * radialT_long q f v := by
  ext ⟨x1, x2, x3, x4⟩
  dsimp [radialT_long, radialT_l_p12, radialT_l_p13, radialT_l_p14, radialT_l_p23, radialT_l_p24, radialT_l_p34]
  ring

theorem radialT_short_const (q : R) (c : R) (v : ApartmentSiteF4) :
    radialT_short q (fun _ => c) v = (4 * (q + 1) + 8 * (q + 1)) * c := by
  dsimp [radialT_short, radialT_s_unit, radialT_s_diag_pos, radialT_s_diag_neg]
  ring

theorem radialT_long_const (q : R) (c : R) (v : ApartmentSiteF4) :
    radialT_long q (fun _ => c) v = (6 * (q^2 + 2 * q + 1)) * c := by
  dsimp [radialT_long, radialT_l_p12, radialT_l_p13, radialT_l_p14, radialT_l_p23, radialT_l_p24, radialT_l_p34]
  ring
...
theorem macdonald_eigenvalue_T_short (S : SatakeSystemF4 R) (ψ : ApartmentSiteF4 → R)
    ...
    radialT_short S.q ψ (x1, x2, x3, x4) = S.q * S.chiShort * ψ (x1, x2, x3, x4) := by
  dsimp [radialT_short, radialT_s_unit, radialT_s_diag_pos, radialT_s_diag_neg,
...
theorem macdonald_eigenvalue_T_long (S : SatakeSystemF4 R) (ψ : ApartmentSiteF4 → R)
    (h : MacdonaldWaveF4 S ψ) (x1 x2 x3 x4 : ℤ) :
    radialT_long S.q ψ (x1, x2, x3, x4) = S.q^2 * S.chiLong * ψ (x1, x2, x3, x4) := by
  dsimp [radialT_long, radialT_l_p12, radialT_l_p13, radialT_l_p14, radialT_l_p23, radialT_l_p24, radialT_l_p34,
