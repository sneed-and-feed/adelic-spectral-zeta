with open('fix_charpoly.lean', 'r', encoding='utf-8') as f:
    code = f.read()
  ...
  | succ m =>
    ...
    | succ n =>
      ...
      · change (-1) ^ (0 : ℕ) * _ * ((charmatrix (Matrix.of fun (i j : Fin (n + 2)) => cyclicWeightMatrix W i j)).submatrix Fin.succ Fin.succ).det + _ = _
        ...
        rw [c0, c_last]
        have c1 : (0 : ZMod (n + 2)) = (1 : ZMod (n + 2)) ↔ False := by
          apply iff_false_intro; intro h
          have h_eq : (1 : ZMod (n + 2)) = (1 : ℕ) := by push_cast; rfl
          rw [h_eq] at h
        ...
        have c1_alt : (0 : ZMod (n + 2)) = 0 + 1 ↔ False := by
          apply iff_false_intro; intro h
          have h' := congrArg ZMod.val h
          change 0 = (0 + 1) % (n + 2) at h'
          omega
        have c2 : (0 : ZMod (n + 2)) = (n + 1 : ZMod (n + 2)) ↔ False := by
          apply iff_false_intro; intro h
          have h_eq : (n + 1 : ZMod (n + 2)) = (n + 1 : ℕ) := by push_cast; rfl
          rw [h_eq] at h
          have h' := congrArg ZMod.val h
      ...
      · intro b _ hb_ne
        ...
        have c1 : (0 : ZMod (n + 2)) = (b : ZMod (n + 2)) ↔ False := by
          ...
          apply hb0
          apply Fin.ext
          have h_eq : (b : ZMod (n + 2)) = (b : ℕ) := by push_cast; rfl
          rw [h_eq] at h
          have h' := congrArg ZMod.val h
...
with open('fix_charpoly.lean', 'w', encoding='utf-8') as f:
    f.write(code)
print("done")
