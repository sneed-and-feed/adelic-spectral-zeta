...
lemma charmatrix_cyclic_submatrix_00 (n : ℕ) (W : ZMod (n + 2) → R) :
  ...
  ext i j n_coeff
  dsimp [charmatrix, Matrix.submatrix_apply, Matrix.sub_apply, Matrix.diagonal_apply, shiftMatrix, cyclicWeightMatrix, Matrix.of]
  ...
  have eq2 : (Fin.succ i : Fin (n + 2)) = (Fin.succ j : Fin (n + 2)) + 1 ↔ (i : ℕ) = (j : ℕ) + 1 := by
    ...
    · intro h
      ...
      omega
  have neq : (i : ℕ) = (j : ℕ) + 1 → i = j → False := by intro h1 h2; subst h2; omega
  simp [eq1, eq2]
  split_ifs <;> simp [*] <;> rfl
...
lemma charmatrix_cyclic_submatrix_0n (n : ℕ) (W : ZMod (n + 2) → R) :
  ...
  ext i j n_coeff
  dsimp [charmatrix, Matrix.submatrix_apply, Matrix.sub_apply, Matrix.diagonal_apply, cyclicWeightMatrix, upperBidiagonal, Matrix.of]
  ...
  have eq2 : (Fin.succ i : Fin (n + 2)) = (Fin.castSucc j : Fin (n + 2)) + 1 ↔ (i : ℕ) = (j : ℕ) := by
    ...
    · intro h
      ...
      rw [Nat.mod_eq_of_lt h_lt]
      omega
  have neq : (i : ℕ) + 1 = (j : ℕ) → (i : ℕ) = (j : ℕ) → False := by omega
  simp [eq1, eq2]
  split_ifs <;> simp [*] <;> rfl
  ...
  | succ m =>
    ...
    | zero =>
      ...
      simp only [Matrix.charpoly, Matrix.det_fin_one, charmatrix, Matrix.sub_apply, Matrix.diagonal_apply,
        Matrix.map_apply, cyclicWeightMatrix, Matrix.of]
