...
In particular, all eigenvalues of the twisted block $S_n$ satisfy $\lvert \lambda \rvert = 2^{2^{-(n-1)}}$.

**Formalization Status in Lean 4:** The inductive block-diagonalization $H D_n H^{-1} = \mathrm{diag}(D_{n-1}, S_n)$, deck commutativity, cyclotomic product identity, and cyclic monomial characteristic polynomials are unconditionally verified with 0 `sorry`s and 0 custom axioms. The global spectral circle eigenvalue magnitude $\lvert \lambda \rvert = 2^{2^{-(n-1)}}$ and absolute spectral gap bound $\lVert \lambda \rVert \le 2^{1/4}$ are formally verified conditionally under `TwistedBlockHypothesis n` and `AbsoluteSpectralGapConjecture` ([`SchreierSpectralGap.lean`](../formalization/Formalization/Spectral/SchreierSpectralGap.lean), [`SpectralCircle.lean`](../formalization/Formalization/Dynamics/SpectralCircle.lean)), guaranteeing zero unverified kernel axioms across the formal codebase.
...
- For $n \ge 3$: $W_{C_1} W_{C_2} = 2$ and $\lvert W_{C_2} \rvert = \lvert W_{C_1} \rvert \implies \lvert W_{C_i} \rvert = \sqrt{2}$.
...
```math
\det(\lambda I - M) = \lambda^m - W \implies \lvert \lambda \rvert = \lvert W \rvert^{1/m}
```

### Eigenvalue Magnitudes for $S_n$:
- For $n = 2$: $m = 2, W = 2 \implies \lvert \lambda \rvert = 2^{1/2} = 2^{2^{-(2-1)}} = \sqrt{2}$.
- For $n \ge 3$: $m = 2^{n-2}, \lvert W \rvert = \sqrt{2} \implies \lvert \lambda \rvert = (\sqrt{2})^{1/2^{n-2}} = 2^{2^{-(n-1)}}$.
...
```math
\lVert \lambda \rVert = 2^{1/2^{n-1}} \le 2^{1/4} = \sqrt[4]{2} < 2 \quad (\forall n \ge 3)
```
...
| $n$ | $\dim(S_n)$ | $2^{2^{-(n-1)}}$ | $\max\lvert\lambda\rvert$ | All on Circle? |
| :---: | :---: | :---: | :---: | :---: |
...
- `TwistedBlockPow.lean`: Exact monomial reduction theorem connecting the 2-cycle orbit structure of $\times 3$ on odd residues to the power identity $S_n^{2^{n-1}} = -2 \mathbf I$ and cyclotomic eigenvalue condition $\lambda^{2L} = -2$.
...
- `SchreierSpectralGap.lean`: Formulates `structure TwistedBlockHypothesis (n : ℕ) : Prop` and `def AbsoluteSpectralGapConjecture : Prop`, proving the conditional absolute spectral gap theorem `absolute_spectral_gap` with bound $(\lVert \lambda \rVert \le 2^{1/4})$.
...
- `ContinuousTransfer.lean` & `SpectralCircle.lean`: Character action lemma, 3-order mod $2^n$, orbit weight magnitude $\lvert W_C \rvert^2 = 2$, and conditional spectral circle magnitude `spectral_circle_magnitude_eq`.
