...
\textbf{Results.} Table~\ref{tab:ring_distances} reports the ensemble-averaged $8 \times 8$ block-distance matrix for reference. Two structural features emerge:

\begin{enumerate}
    \item \textbf{Intra-domain clustering.} Blocks from the same semantic domain exhibit systematically lower ensemble distances than cross-domain pairs. The Natural Language blocks $(B_0, B_1)$ drop to $\bar{d}_p = 7.9$, and the HTML blocks $(B_6, B_7)$ drop to $\bar{d}_p = 7.9$. By contrast, the cross-domain Code--NL pair $(B_0, B_2)$ reaches $\bar{d}_p = 9.1$.
    \item \textbf{Per-Head Sparsity Savings.} Standard dense Ring Attention on 8 GPUs with 32 heads requires $H \cdot N^2 = 2{,}048$ P2P communication edges. By applying a standard pruning threshold of $\tau = 0.75 \cdot L = 8.25$ \emph{independently to each head}, the network prunes all edges where a specific head finds no semantic relevance between two blocks. This per-head evaluation preserves the full uncompressed dynamic range of the individual trees. Under this threshold, the router natively prunes the cluster down to $448$ active communication edges, achieving a \textbf{78.1\% reduction in P2P network bandwidth} without discarding semantically relevant context.
\end{enumerate}
