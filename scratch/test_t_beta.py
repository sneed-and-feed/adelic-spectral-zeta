...
def main():
    ...
    # We define beta(t) pointwise: Re(G_off)/F_var and Im(G_off)/F_var
    # Let's fit a linear function: beta_real(t) = w0 + w1 * t
    ratio_real = np.real(G_off_vals) / F_vals
    ratio_imag = np.imag(G_off_vals) / F_vals
    
    w0_real, w1_real = np.polyfit(exact_zeros, ratio_real, 1) # w0*t + w1
    w0_imag, w1_imag = np.polyfit(exact_zeros, ratio_imag, 1)
    
    print(f"beta_real(t) = {w0_real:.4f} * t + {w1_real:.4f}")
    print(f"beta_imag(t) = {w0_imag:.4f} * t + {w1_imag:.4f}")
    
    # y_pred with t-dependent beta(t)
    beta_real_t = w0_real * exact_zeros + w1_real
    beta_imag_t = w0_imag * exact_zeros + w1_imag
    
    G_off_pred = (beta_real_t * F_vals) + 1j * (beta_imag_t * F_vals)
    y_pred = C_mag_vals * H_vals / np.abs(G_diag_vals + G_off_pred)
    slope_pred, _ = np.polyfit(F_vals, y_pred, 1)
    
    print("\nComparing y_vals and y_pred (t-dependent beta):")
