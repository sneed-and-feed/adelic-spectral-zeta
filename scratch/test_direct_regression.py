...
def main():
    ...
    # Let's perform linear fit of Re(G_off) = beta * F_var
    beta_fit, _ = np.polyfit(F_vals, np.real(G_off_vals), 1)
    
    # 1. Exact direct regression slope
    # Since y_pred_k = (C_mag_k * H_k) / |G_diag_k + beta * F_var_k|
    # Note that G_diag_k and G_off_k have complex values.
    # What if we use G_off_pred_k = beta * F_var_k (projected onto the phase of G_off_k)?
    # Or simply y_pred_k = C_mag_k * H_k / |G_diag_k + beta_fit * F_var_k|?
    # Let's check both:
    
    # Case A: y_pred = C_mag * H / |G_diag + beta_fit * F_var|
    # Note: beta_fit is real.
    y_pred_A = C_mag_vals * H_vals / np.abs(G_diag_vals + beta_fit * F_vals)
    slope_A, _ = np.polyfit(F_vals, y_pred_A, 1)
    
    # Case B: y_pred = C_mag * H / |G_diag + G_off| (which is exactly y_vals!)
    slope_B, _ = np.polyfit(F_vals, y_vals, 1)
    
    print(f"Actual Slope from y_vals: {slope_B:.4f}")
    print(f"Predicted Slope from Case A (direct substitution): {slope_A:.4f}")
    print(f"Relative error Case A: {abs(slope_A - slope_B) / abs(slope_B) * 100:.4f}%")
