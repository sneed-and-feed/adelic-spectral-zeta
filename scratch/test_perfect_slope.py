...
def main():
    ...
    print(f"Mean <C_mag * H>: {mean_C_H:.6f}")
    print(f"Mean <|G_diag + G_off|>: {mean_G_total:.6f}")
    print(f"Fitted beta (Re(G_off) / F_var): {beta_fit:.4f}")
    print(f"Predicted Slope (c1): {predicted_slope:.4f}")
    print(f"Actual Numerical Slope: -592.3204")
    print(f"Relative error: {abs(predicted_slope - (-592.3204)) / 592.3204 * 100:.2f}%")
