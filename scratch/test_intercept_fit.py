...
def main():
    ...
    beta_fit, alpha_fit = np.polyfit(F_vals, np.real(G_off_vals), 1)
    print(f"Fitted: Re(G_off) = {beta_fit:.4f} * F_var + {alpha_fit:.4f}")
    
    # Let's compare y_vals and y_pred with the intercept included!
    y_pred_with_intercept = C_mag_vals * H_vals / np.abs(G_diag_vals + (beta_fit * F_vals + alpha_fit))
    slope_pred, _ = np.polyfit(F_vals, y_pred_with_intercept, 1)
    
    print("\nComparing y_vals and y_pred_with_intercept:")
    for i in range(len(exact_zeros)):
        print(f"t = {exact_zeros[i]:.4f}: y_actual = {y_vals[i]:.4f}, y_pred = {y_pred_with_intercept[i]:.4f}")
        
    print(f"\nSlope from actual y_vals: {np.polyfit(F_vals, y_vals, 1)[0]:.4f}")
    print(f"Slope from predicted y: {slope_pred:.4f}")
    print(f"Relative error: {abs(slope_pred - np.polyfit(F_vals, y_vals, 1)[0]) / abs(np.polyfit(F_vals, y_vals, 1)[0]) * 100:.4f}%")
