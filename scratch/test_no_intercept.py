...
def main():
    ...
    # Fit without intercept: Re(G_off) = beta_real * F_var
    # To do this, we solve the least squares: beta = sum(x * y) / sum(x^2)
    beta_real = np.sum(F_vals * np.real(G_off_vals)) / np.sum(F_vals**2)
    beta_imag = np.sum(F_vals * np.imag(G_off_vals)) / np.sum(F_vals**2)
    
    print(f"Fitted Real (no intercept): Re(G_off) = {beta_real:.4f} * F_var")
    print(f"Fitted Imag (no intercept): Im(G_off) = {beta_imag:.4f} * F_var")
    
    # y_pred with both fits (no intercept)
    G_off_pred = (beta_real * F_vals) + 1j * (beta_imag * F_vals)
    y_pred = C_mag_vals * H_vals / np.abs(G_diag_vals + G_off_pred)
    slope_pred, _ = np.polyfit(F_vals, y_pred, 1)
    
    print("\nComparing y_vals and y_pred (no intercept):")
