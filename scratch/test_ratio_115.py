...
# Let's run with lambda_val = 1.15
D0_eigs, Dglob_eigs = compute_eigenvalues(N_dim=150, lambda_val=1.15, p_max=151)
...
for t in t_arr:
    ...
    # Compute Weierstrass product of D0
    ...
    ratios.append(ratio_glob)
    print(f"t={t:.2f} | val_L={val_L:.6f} | val_glob={np.abs(val_glob):.6f} | ratio_glob={ratio_glob:.6f}")

ratios = np.array(ratios)
print(f"rel_std = {np.std(ratios)/np.mean(ratios):.6f}")
