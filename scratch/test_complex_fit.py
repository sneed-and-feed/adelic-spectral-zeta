...
def main():
    ...
    F_vals = np.abs(F_complex)
    
    # Fit complex beta and alpha: G_off = beta_c * F_complex + alpha_c
    # This is a complex linear fit. Since complex division works, we can fit
    # by treating real and imaginary parts of F_complex as independent or
    # using complex polyfit.
    # To fit G_off = beta_c * F_complex + alpha_c:
    beta_c, alpha_c = np.polyfit(F_complex, G_off_vals, 1)
    print(f"Fitted complex model: G_off = ({beta_c:.4f}) * F_complex + ({alpha_c:.4f})")
    
    G_off_pred = beta_c * F_complex + alpha_c
    y_pred = C_mag_vals * H_vals / np.abs(G_diag_vals + G_off_pred)
    slope_pred, _ = np.polyfit(F_vals, y_pred, 1)
    
    print("\nComparing y_vals and y_pred (complex fit):")
